...
def test_render_hash_stable():
    spec = RenderSpec(formula_id="polar_loom", width=64, height=64,
                      params={"rings": 8, "twist": 2.0})
...
def test_checksum_sha256_hex():
    r, _, _ = render(RenderSpec(formula_id="polar_loom", width=48, height=48,
                                params={"rings": 6}))
    assert len(r.checksum) == 64
    assert all(c in "0123456789abcdef" for c in r.checksum)

def test_preview_smaller():
    r, full, prev = render(RenderSpec(formula_id="polar_loom", width=1024,
                                      height=768, params={"rings": 12}))
